...
void InsertToWaitQueue(int mqd, Thread *tcb){
    ...
    if(curQcb->pWaitQHead == NULL){
        curQcb->pWaitQHead = tcb;
        curQcb->pWaitQTail = tcb;
        tcb->phNext = NULL;
        tcb->phPrev = NULL;
    }
    else{//insert to Tail
        tcb->phNext = curQcb->pWaitQTail;
        curQcb->pWaitQTail->phPrev = tcb;
        curQcb->pWaitQTail = tcb;
    }
    tcb->status = THREAD_STATUS_WAIT;
    curQcb->waitThreadCount++;
}
/*
*   mqd 입력받아 mqd에 해당하는 테이블의 waitQueue head를 제거
*   waitQueue count--, Thread 상태 ready
*/
Thread* DeleteToWaitQueueHead(int mqd){
    Qcb *curQcb = qcbTblEntry[mqd].pQcb;
    Thread* temp;
    //list empty
    if((curQcb->pWaitQHead == NULL) && (curQcb->pWaitQTail == NULL)){
        return NULL;
    }
    //In list one Thread
    if(curQcb->pWaitQHead == curQcb->pWaitQTail){
        temp = curQcb->pWaitQTail;
        curQcb->pWaitQHead = NULL;
        curQcb->pWaitQTail = NULL;
    }
    //more than 2
    else{
        temp = curQcb->pWaitQHead;
        curQcb->pWaitQHead = temp->phPrev;
        temp->phPrev->phNext = NULL;
        temp->phPrev = NULL;
    }
    temp->status = THREAD_STATUS_READY;
    curQcb->waitThreadCount--;
    return temp;
}
